...
int main(int argc, char *argv[]) 
{
    ...
    current_req=req_head=read_request(ssd, requests);
    
    display_simulation_intro(ssd);
    while (current_req != NULL) {
        latency=process_io(ssd, &req_head, current_req);

        printf("Latency for %lld is %lld\n", current_req->time, latency);
        current_req = current_req->next_node;
    }
...
/***************************************************************
 * Read all request from trace file and insert it to our 
 * requests queue. We can assume our request queue as another
 * "trace file"
 **************************************************************/
struct request *read_request(struct ssd_info *ssd, struct request* requests)
{
    char buffer[200];
    int r_device, r_size;
    unsigned int r_lsn = 0, r_operation;
    int64_t r_time = 0;
    struct request *req, *requests_tail;
    ...
    while(fgets(buffer,200,ssd->tracefile)) {
        sscanf(buffer, "%lld %d %d %d %d", &r_time, &r_device, &r_lsn, &r_size, &r_operation);
        trace_assert(r_time, r_device, r_lsn, r_size, r_operation);
        ...
        req->lsn = r_lsn;
        req->size = r_size;
        req->operation = r_operation;
        req->begin_time = r_time;
        req->response_time = 0;
        req->energy_consumption = 0;
        req->next_node = NULL;
        req->distri_flag = 0;
        req->subs = NULL;
        req->need_distr_flag = NULL;
        req->complete_lsn_count=0;
        req->next_node = NULL;
...
/****************************************************************************************************
 * Assumption: all the request in the simulation will finish in fifo order
 * process_io() is the main simulation function
 * request_head is the current pointer in "trace file", instead of using file, we use queue
 * current_req is the request which the latency is returned from this function
 ****************************************************************************************************/
int64_t process_io(struct ssd_info *ssd, struct request **requests_head, struct request *current_req)
{
    unsigned int flag = 0;
    struct request *req;
    int64_t latency = -1;

    // Run simulation until first request (current_request) is done, 
    // indicated with latency >= 0
    while (latency < 0) {
        req = *requests_head;
        flag = try_insert_request(ssd, req);

        if (flag == FLAG_NEW_REQUEST_ADDED) {
            *requests_head = (*requests_head)->next_node;
...
/***************************************************************
 * This function will try to insert new request from 
 * our request queue (our "trace file") into ssd request queue.
 * Return value:
 *   FLAG_NO_REQUEST_ADDED  : our new request can not be inserted 
 *                            to ssd queue because of some reasons
 *   FLAG_NEW_REQUEST_ADDED : our new request is successfully 
 *                            added to ssd request queue
 ***************************************************************/
int try_insert_request(struct ssd_info *ssd, struct request *req)
{
    int large_lsn;
    ...
    if(req == NULL) {
        ...
        return FLAG_NO_REQUEST_ADDED;
    }

    if(req->lsn < 0 && req->size < 0 && req->operation < 0) {
        printf("Error! wrong io request from trace file\n");
        exit(1);
    }

    if(req->lsn < ssd->min_lsn) ssd->min_lsn = req->lsn;
    if(req->lsn > ssd->max_lsn) ssd->max_lsn = req->lsn;
    ...
    if (nearest_event_time == MAX_INT64) {
        ...
        if (nearest_event_time < req->time) {
    ...
    if (req->time < 0) {
        printf("Error! time is less than 0");
        exit(1);
    }

    
    // Successfully add new request to ssd request queue, malloc new request object, then insert it to request queue
    new_req = (struct request*)malloc(sizeof(struct request));
    alloc_assert(new_req,"request");
    memset(new_req,0, sizeof(struct request));

    new_req->time = req->time;
    new_req->lsn = req->lsn;
    new_req->size = req->size;
    new_req->operation = req->operation;
    new_req->begin_time = req->time;
    new_req->response_time = 0;
    new_req->energy_consumption = 0;
    new_req->next_node = NULL;
    new_req->distri_flag = 0;              // indicate whether this request has been distributed already
    new_req->subs = NULL;
    new_req->need_distr_flag = NULL;
    new_req->complete_lsn_count=0;         //record the count of lsn served by buffer

    if(ssd->request_queue == NULL) {
        ssd->request_queue = new_req;
        ssd->request_tail = new_req;
        ssd->request_queue_length++;
    } else {
        (ssd->request_tail)->next_node = new_req;
        ssd->request_tail = new_req;
        ssd->request_queue_length++;
    }
    ...
    if (req->next_node != NULL)
        ...
        ssd->next_request_time = req->time;

    return FLAG_NEW_REQUEST_ADDED;
}
