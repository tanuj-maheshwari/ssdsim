 ...
 *4，输出每条请求的子请求都处理完后的相关信息到outputfile文件中
 --------------------------------------------------------------------------------------------------
 * simulate() is the core processing function, and the main functions include:
 *1, Get a request from the trace file and hang it to ssd->request
 *2, according to whether the ssd has dram to process the read requests separately, process these requests into read and write sub-requests, and hang them on ssd->channel or ssd
 *3, these read and write sub-requests are processed according to the sequence of events.
 *4, output the relevant information after the sub-request of each request is processed to the outputfile file
 **************************************************************************************************/
...
int get_requests(struct ssd_info *ssd)
{
    ...
    if (lsn > ssd->max_lsn)
        ssd->max_lsn = lsn;
    /******************************************************************************************************
     * Any read and write commands sent by the upper file system to the SSD include two parts (LSN, size) LSN is the logical sector number.
     * Storage space is a linear continuous space. For example, the read request (260, 6) indicates that it needs to read the logical sector starting from the sector number 260, a total of 6 sectors.
    ...
    else
    {
        ...
        {
            /*******************************************************************************
             * Rollback, that is, if time_t is not assigned to ssd->current_time, a record read in the trace file will be rolled back
             * filepoint records the file pointer position before executing fgets, and rolls back to the file header + filepoint
             * int fseek(FILE *stream, long offset, int fromwhere); The function sets the position of the file pointer stream.
             *  If the execution is successful, stream will point to fromwhere (offset starting position: file head 0, current position 1, file tail 2) as the reference,
             *  The position at offset (pointer offset) bytes. If the execution fails (such as the offset exceeds the size of the file itself), the location pointed to by the stream is not changed.
             * Text files can only use the positioning method of file header 0. In this program, the file opening method is "r": open the text file in read-only mode
             **********************************************************************************/
    ...
    if (request1->operation == READ) // Calculate the average request size 1 to read 0 to write
    {
    ...
    filepoint = ftell(ssd->tracefile);
    fgets(buffer, 200, ssd->tracefile); // Find the time for the next requestrival time of the next request
    sscanf(buffer, "%lld %d %d %d %d", &time_t, &device, &lsn, &size, &ope);
...
/**********************************************************************************************************************************************
 * First of all, the buffer is a write buffer, which is to serve the write request, because the time tR for reading the flash is 20us, and the time for writing the flash tprog is 200us, so it can save time for the write service.
 * Read operation:  If the buffer is hit, read from the buffer, do not occupy the I/O bus of the channel, if the buffer is not hit, read from the flash, occupy the I/O bus of the channel, but do not enter the buffer.
 * Write operation: First, the request is divided into sub_request sub-requests. If it is dynamically allocated, sub_request is linked to ssd->sub_request, because it is unknown which channel's sub_request to be linked to first.
 *                  If it is statically allocated, the sub_request is linked to the sub_request chain of the channel, and whether the sub_request is dynamically allocated or statically allocated, it must be linked to the sub_request chain of the request
 *                  Because every time a request is processed, information about the request must be output in the traceoutput file. After processing a sub_request, remove it from the channel's sub_request chain
 *                  Or remove the sub_request chain of the ssd, but clear the sub_request chain of the request after the traceoutput file outputs one.
 *                  If the sub_request hits the buffer, just write it in the buffer, and refer the sub_page to the buffer chain head (LRU). If it does not hit and the buffer is full, then the sub_request in the buffer chain tail is first
 *                  Write to flash (this will generate a sub_request write request, which will be linked to the sub_request chain of this request request, and it will be linked to the channel or ssd depending on whether it is dynamically allocated or statically allocated.
 *                  sub_request chain), write the sub_page to be written to the buffer chain head
 ***********************************************************************************************************************************************/
...
float get_crt_free_page_prct(struct ssd_info *ssd)
{
    ...
    for (i = 0; i < ssd->parameter->channel_number; i++)
    {
        page_total += (((int64_t)ssd->parameter->page_block * ssd->parameter->block_plane * ssd->parameter->plane_die * ssd->parameter->die_chip * ssd->parameter->chip_channel[i] * (ssd->parameter->block_chunk - 1)) / ssd->parameter->block_chunk);
...
float get_crt_nonempty_free_block_prct(struct ssd_info *ssd)
{
    ...
    for (i = 0; i < ssd->parameter->channel_number; i++)
    {
        blk_total += (((int64_t)ssd->parameter->block_plane * ssd->parameter->plane_die * ssd->parameter->die_chip * ssd->parameter->chip_channel[i] * (ssd->parameter->block_chunk - 1)) / ssd->parameter->block_chunk);
...
/*****************************************************************************
 * The role of the make_aged () function is to simulate the real SSD for a period of time,
 * Then the corresponding parameters of this SSD must be changed, so this function is essentially the assignment to each parameter in SSD.
 * Sets the first `aged_ratio` ratio of pages of each block as invalid
 ******************************************************************************/
struct ssd_info *make_aged(struct ssd_info *ssd)
{
    ...
    {
        // threshold indicates how many pages in a plane need to be invalidated in advance
                ...
                for (k = 0; k < ssd->parameter->die_chip; k++)
                    for (l = 0; l < ssd->parameter->plane_die; l++)
                    {
                        ...
                        for (m = 0; m < ssd->parameter->block_plane; m++)
                        {
                            ...
                            {
                                ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].valid_state = 0; // Indicates that a page is invalid, and the valid and free states are both marked as 0
                                ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].free_state = 0;  // Indicates that a page is invalid, and the valid and free states are both marked as 0
                                ssd->channel_head[i].chip_head[j].die_head[k].plane_head[l].blk_head[m].page_head[n].lpn = 0;         // Set valid_state free_state lpn to 0 to indicate that the page is invalid. When testing, all three items are detected. Lpn=0 alone can be a valid page.
...
void close_file(struct ssd_info *ssd)
{
    ...
    if (ssd->statisticfile)
        fclose(ssd->statisticfile);
    if (ssd->statisticfile2)
        fclose(ssd->statisticfile2);
    if (ssd->outfile_gc)
        fclose(ssd->outfile_gc);
}
            ...
            for (die = 0; die < ssd->channel_head[channel].chip_head[chip].die_num; die++)
                for (plane = 0; plane < ssd->parameter->plane_die; plane++)
                    for (block = 0; block < ssd->parameter->block_plane; block++)
                    {
                        ...
                        total_bk++;
                        if (valid_pg == ssd->parameter->page_block)
                            total_full_vbk++;
                        if (free_pg == ssd->parameter->page_block)
                            total_empty_bk++;
                    }
...
struct ssd_info *init_gc(struct ssd_info *ssd)
{
    ...
    threshold = ((int64_t)ssd->parameter->page_block * ssd->parameter->block_plane * ssd->parameter->plane_die * ssd->parameter->die_chip * ssd->parameter->chip_num * (1 - ssd->parameter->overprovide) * ssd->parameter->gc_hard_threshold * (ssd->parameter->block_chunk - 1)) / ssd->parameter->block_chunk;
    free_page = 0;
    for (channel = 0; channel < ssd->parameter->channel_number; channel++)
    {
        for (chip = 0; chip < ssd->channel_head[channel].chip; chip++)
